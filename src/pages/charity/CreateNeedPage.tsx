...
import api, { needsAPI, charityAPI } from "../../services/api";
...
const URGENCIES = [
  ...
  { value: "high", label: "High", desc: "Needed within weeks" },
  {
    value: "critical",
    label: "Critical 🚨",
    desc: "Immediate action required — sends push alerts",
  },
];
...
export default function CreateNeedPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.targetAmount || Number(form.targetAmount) < 100)
      return toast.error("Target amount must be at least ₹100");
    ...
    try {
      let needId: string = "";
      if (isEdit) {
        const r = await needsAPI.update(id!, payload);
        needId = r.data.data._id;
      } else {
        const r = await needsAPI.create(payload);
        needId = r.data.data._id;
      }
      ...
      if (images.length > 0) {
        ...
        await api.post(`/uploads/needs/${needId}`, fd, {
    ...
    return (
      <div className="container mx-auto px-4 py-20 max-w-md text-center">
        ...
        <p className="text-muted-foreground">
          Your charity must be verified by an admin before you can list needs.
        </p>
      </div>
    );
    ...
    <div className="container mx-auto px-4 py-8 max-w-2xl">
      ...
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-1">
          {isEdit
            ? "Update your need details — it will be re-reviewed by admin."
            : "Describe what your charity needs. It will go live after admin approval."}
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {form.urgency === "critical" && (
            <div className="mt-2 flex items-start gap-2 bg-red-50 border border-red-200 rounded-xl p-3">
              ...
              <p className="text-xs text-red-700">
                Critical needs will trigger push notifications to nearby donors
                when approved.
              </p>
            </div>
          )}
        ...
        <p className="text-xs text-center text-muted-foreground">
          After submission, an admin will review and approve your listing before
          it goes live.
        </p>
      </form>
    </div>
