...
export default function NeedDetailPage() {
        ...
        if (isAuthenticated) {
          recommendAPI
            .trackInteraction({
              ...
              charityId: r.data.data.charity?._id,
              action: "view",
            })
            .catch(() => {});
        }
  ...
  const progress =
    need.targetAmount > 0
      ? Math.min((need.raisedAmount / need.targetAmount) * 100, 100)
      : 0;
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl page-in">
        ...
        <div className="lg:col-span-2 space-y-5">
          {isCritical && (
            <div className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-2xl px-4 py-3">
              ...
              <p className="text-sm font-semibold text-red-700">
                🚨 CRITICAL — This need requires immediate support
              </p>
            </div>
          )}
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2 mb-3">
              ...
              <span
                className={`px-2.5 py-0.5 rounded-full text-xs font-semibold border ${getUrgencyColor(need.urgency)}`}
              >
                {need.urgency}
              </span>
            ...
            {need.charity && (
              ...
              >
                <div className="w-7 h-7 rounded-lg bg-saffron-100 flex items-center justify-center text-sm font-bold text-saffron-700 overflow-hidden">
                  {need.charity.logo ? (
                    <img
                      src={need.charity.logo}
                      alt={need.charity.name}
                      className="w-full h-full object-cover"
                    />
                  ) : (
                    need.charity.name?.charAt(0)
                  )}
                </div>
                <span className="text-sm font-medium text-foreground group-hover:text-saffron-600 transition-colors">
                  {need.charity.name}
                </span>
                {need.charity.isVerified && (
          ...
          <div className="bg-card border border-border rounded-2xl p-5">
            <h2 className="font-display font-semibold text-foreground mb-3 text-sm uppercase tracking-wider text-muted-foreground">
              About this need
            </h2>
          ...
          {need.beneficiaryDescription && (
            <div className="bg-blue-50 border border-blue-100 rounded-2xl p-5">
              ...
              {need.beneficiaryCount > 0 && (
                <p className="text-sm font-bold text-blue-700 mt-1.5">
                  {need.beneficiaryCount} people will benefit
                </p>
              )}
            </div>
          )}
          ...
          {need.images?.length > 0 && (
            <div className="bg-card border border-border rounded-2xl p-5">
              <h2 className="font-display font-semibold mb-3 text-sm uppercase tracking-wider text-muted-foreground flex items-center gap-2">
                <ImageIcon className="w-4 h-4" /> Need images
              </h2>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {need.images.map((img: string, idx: number) => (
                  <button
                    key={`${img}-${idx}`}
                    type="button"
                    onClick={() => openPreview(need.images, idx)}
                    className="rounded-xl overflow-hidden border border-border bg-background group relative focus:outline-none focus:ring-2 focus:ring-saffron-400"
                  >
                    <img
                      src={`${API_BASE}${img}`}
                      alt={`Need preview ${idx + 1}`}
                      className="w-full h-40 object-cover transition-transform duration-200 group-hover:scale-105"
                      onError={(e) => {
                        (e.target as HTMLImageElement).src =
                          "https://placehold.co/400x160/f3f4f6/9ca3af?text=Image";
                      }}
                    />
          ...
          <div className="flex flex-wrap gap-4 text-sm text-muted-foreground pt-2 border-t border-border">
            {need.charity?.location?.city && (
              <span className="flex items-center gap-1.5">
                <MapPin className="w-4 h-4" /> {need.charity.location.city}
              </span>
            )}
            {need.deadline && (
            ...
            <span className="flex items-center gap-1.5">
              <Users className="w-4 h-4" /> {need.donorCount || 0} donors
            </span>
            <button
              onClick={() => {
      ...
      {/* Global Extracted Modal Placement */}
      {previewImages && (
        <ImagePreviewModal
